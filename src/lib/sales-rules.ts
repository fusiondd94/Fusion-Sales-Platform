...
export type BusinessRules = {
  ...
  /** Phase 4: shown alongside estimated portal-product costs - covers the
   * fact that final portal pricing, taxes, renewal rates, and promotions
   * are confirmed at checkout, not guaranteed by this estimate. */
  portalPricingDisclaimerText: string;
...
export function resolveBusinessRules(rows: BusinessRuleRow[]): BusinessRules {
  ...
  return {
    ...
    portalPricingDisclaimerText: textField(
      ...
      "text",
      "Portal product prices are estimates and may change. Pricing excludes applicable taxes and ICANN fees. Final pricing, renewal rates, and any active promotions are confirmed at checkout in the Fusion client portal."
    ),
    version: maxVersion
  };
}
...
/**
 * Phase 4: the live-ish portal product catalog. There is no supported
 * pricing API for the underlying GoDaddy Pro reseller storefront
 * (portal.fddynamics.com renders pricing client-side with no public feed),
 * so per the Phase 4 fallback this is an administrator-managed table -
 * prices are manually verified against the storefront and stamped with
 * last_verified_at rather than scraped live. Never hard-code a price here;
 * every dollar figure the recommendation engine sees comes from this table.
 */
export async function loadPortalProductCatalog(organizationId?: string) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, product_key, product_name, category, description, estimated_price, price_unit, is_required_default, portal_url, is_active, sort_order, renewal_price_note, tax_included, icann_fee_applies, last_verified_at, recommended_use_cases, includes_ssl, includes_email, includes_backup"
    )
    .eq("organization_id", orgId)
    .eq("is_active", true)
