...
function estimateMinutes(totalQuestions: number): string {
  const minutes = Math.max(3, Math.round((totalQuestions * 12) / 60));
...
export function QuestionnaireFlow({ initialState }: { initialState: QuestionnaireState | null }) {
  ...
  useEffect(() => {
    ...
    generateRecommendationAction().then((result) => {
      if (result.ok) {
        if (result.recommendation.resultToken) {
          window.location.href = `/results/${result.recommendation.resultToken}`;
          return;
        }
  ...
  return (
    <section className="flow-panel">
      ...
      <p className="muted questionnaire-time-estimate">
        Takes about {estimateMinutes(state.progress.totalVisible)} to complete - {state.progress.answeredCount} of {state.progress.totalVisible} answered so far
      </p>
